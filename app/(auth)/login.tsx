...
import { livreurService } from '@/services';
...
export default function LoginScreen() {
  ...
  async function handleLogin() {
    if (!email.trim() || !token.trim()) {
      Alert.alert('Champs requis', 'Veuillez renseigner votre email et votre code.');
      return;
    }
    setLoading(true);
    try {
      const livreur = await livreurService.login(email.trim(), token.trim());
      if (!livreur) {
